...
use syntax_pos::{self, Span, Pos, BytePos};
    ...
    pub fn parse_ty(&mut self) -> PResult<'a, P<Ty>> {
        ...
        if let TyKind::Path(None, path) = lhs.node {
            ...
            let hi = bounds.iter().map(|x| match *x {
                TraitTyParamBound(ref tr, _) => tr.span,
                RegionTyParamBound(ref r) => r.span,
            }).max_by_key(|x| x.hi.to_usize());
            let full_span = hi.map(|hi| lhs.span.to(hi));
    ...
    pub fn parse_ty_no_plus(&mut self) -> PResult<'a, P<Ty>> {
        ...
        let t = if self.eat(&token::OpenDelim(token::Paren)) {
            ...
            if self.eat(&token::Not) {
                ...
                let hi = self.span;
                TyKind::Mac(respan(lo.to(hi), Mac_ { path: path, tts: tts }))
            } else {
                // NAMED TYPE
