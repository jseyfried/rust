    ...
    where T: MacroGenerable,
{
    ...
    let configured = expanded.fold_with(&mut fld.strip_unconfigured());
    let marked =
        configured.fold_with(&mut Marker { mark: mark, expn_id: Some(fld.cx.backtrace()) });
