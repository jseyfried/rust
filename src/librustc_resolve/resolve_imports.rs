...
impl<'a, 'b:'a, 'tcx:'b> ImportResolver<'a, 'b, 'tcx> {
    ...
    fn resolve_single_import(&mut self,
        ...
        for &(ns, result) in &[(ValueNS, &value_result), (TypeNS, &type_result)] {
            if let Success(binding) = *result {
                ...
                if !binding.is_public() &&
                   !target_module.is_ancestor_of(self.resolver.current_module) {
                    privacy_error = Some(Box::new(PrivacyError(directive.span, source, binding)));
                } else {
                    privacy_error = None;
        ...
        if report_privacy_error { // then all successful namespaces are privacy errors
            ...
            self.resolver.privacy_errors.insert(*privacy_error.unwrap());
        }
