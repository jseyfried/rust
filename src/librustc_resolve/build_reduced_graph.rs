...
impl<'a> Resolver<'a> {
    ...
    fn build_reduced_graph_for_item(&mut self, item: &Item, expansion: Mark) {
        ...
        match item.node {
            ...
            ItemKind::ExternCrate(_) => {
                ...
                let module = self.get_module(DefId { krate: crate_id, index: CRATE_DEF_INDEX });
            ...
            ItemKind::Enum(ref enum_definition, _) => {
                ...
                let kind = ModuleKind::Def(def, ident.name);
                let module =
                    self.new_module(parent, kind, parent.normal_ancestor_id, expansion, item.span);
                self.define(parent, ident, TypeNS, (module, vis, sp, expansion));
            ...
            ItemKind::Trait(..) => {
                ...
                let kind = ModuleKind::Def(Def::Trait(def_id), ident.name);
                let module =
                    self.new_module(parent, kind, parent.normal_ancestor_id, expansion, item.span);
                self.define(parent, ident, TypeNS, (module, vis, sp, expansion));
                self.current_module = module;
    ...
    fn build_reduced_graph_for_block(&mut self, block: &Block, expansion: Mark) {
        ...
        if self.block_needs_anonymous_module(block) {
            let kind = ModuleKind::Block(block.id);
            let module =
                self.new_module(parent, kind, parent.normal_ancestor_id, expansion, block.span);
    ...
    fn build_reduced_graph_for_external_crate_def(&mut self, parent: Module<'a>, child: Export) {
        ...
        match def {
            Def::Mod(..) | Def::Enum(..) => {
                let kind = ModuleKind::Def(def, ident.name);
                let module = self.new_module(parent, kind, def_id, expansion, DUMMY_SP);
            ...
            Def::Trait(..) => {
                let kind = ModuleKind::Def(def, ident.name);
                let module =
                    self.new_module(parent, kind, parent.normal_ancestor_id, expansion, DUMMY_SP);
                self.define(parent, ident, TypeNS, (module, vis, DUMMY_SP, expansion));
