...
impl TokenStream {
    ...
    /// Returns an iterator over `TokenTree`s
    #[unstable(feature = "proc_macro_lib", issue = "38356")]
    pub fn trees(&self) -> impl Iterator<Item = TokenTree> {
        Cursor { cursor: self.0.trees(), next: None }
    }
}
...
pub mod __internal {
    ...
    pub fn token_stream_inner<T: Into<TokenStream>>(stream: T) -> tokenstream::TokenStream {
        stream.into().0
    }
